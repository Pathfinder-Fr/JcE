...
var character = {
    ...
    renderDeckCardSelector: function () {
        ...
        // we add all cards to a single array, which we will sort by text
        var allCards = [];
        $.each(dataset.cards[selectorType], function (sourceIndex, value) {
            var suffix = value.source[0].toUpperCase();
            if (value.source.indexOf("aventure") == 0) {
                suffix = value.source[9];
            }

            $.each(value.cards, function(index, text) {
                allCards.push({ sourceIndex: sourceIndex, source: [suffix], index: index, text: text, sortText: removeDiacritics(text).toLowerCase() });
            });
        });

        allCards.sort(function (c1, c2) {
            if (c1.sortText > c2.sortText) return 1;
            if (c1.sortText < c2.sortText) return -1;
            if (c1.sourceIndex > c2.sourceIndex) return 1;
            if (c1.sourceIndex < c2.sourceIndex) return -1;
            return 0;
        });
        ...
        while (i >= 0) {
            var item = allCards[i];
            if (previous && previous.sortText === item.sortText) {
                allCards.splice(i + 1, 1);
                item.source = item.source.concat(previous.source);
            }
        ...
        $.each(allCards, function(_, card) {
            var cardValue = card.sourceIndex * editor.cardSourceOffset + card.index;

            selector.append($("<option>")
                .attr("value", cardValue)
                .text(card.text + " (" + card.source.join(", ") + ")")
